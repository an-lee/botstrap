...
- Phase 3: Unix writes `~/.config/botstrap/env.sh` and appends PATH snippet to `.zshrc`/`.bashrc`; Windows appends `# botstrap PATH` (and related blocks) to **each** managed PowerShell profile via `lib/profile-windows.ps1`: `Documents\WindowsPowerShell\Microsoft.PowerShell_profile.ps1`, `Documents\PowerShell\Microsoft.PowerShell_profile.ps1`, plus `$PROFILE` when distinct (`bin/botstrap.ps1` + `botstrap` function). `botstrap` is available after a new shell
