...
| `~/.config/botstrap/env.sh` | **Unix Phase 3:** sets **`BOTSTRAP_ROOT`** and prepends **`$BOTSTRAP_ROOT/bin`** to **`PATH`** (duplicate-safe). On **Linux**, also conditionally prepends **`/opt/nvim-linux-x86_64/bin`** (x86_64) or **`/opt/nvim-linux-arm64/bin`** (arm64) when the Neovim binary is present there. Regenerated each Phase 3 run. |
